    ...
    try:
        ...
        if month_btn.is_visible():
            ...
            if month_option.is_visible():
                ...
                # Target the exact ASP.NET LinkButton generated by GDV (lbFilter)
                filter_link = page.locator("a[id*='lbFilter']").first

                if filter_link.count() > 0 and filter_link.is_visible():
                    logging.info(f"Clicking ASP.NET LinkButton filter target: {filter_link.get_attribute('id')}")
                    filter_link.click()
                else:
                    logging.info("LinkButton not found, triggering form postback...")
